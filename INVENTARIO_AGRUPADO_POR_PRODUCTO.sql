-- =====================================================
-- INVENTARIO B2C AGRUPADO POR PRODUCTO
-- =====================================================
-- Agrupa variantes del mismo producto en una sola tarjeta
-- =====================================================

-- 1. Crear el tipo agrupado
DROP TYPE IF EXISTS inventario_b2c_producto_agrupado CASCADE;

CREATE TYPE inventario_b2c_producto_agrupado AS (
  product_id UUID,
  ...
  variantes JSONB  -- Array de variantes
);

-- 2. Crear función agrupada
...
BEGIN
  ...
  WITH variantes_agrupadas AS (
    SELECT 
      ...
      SUBSTRING(MAX(o.id)::TEXT FROM 1 FOR 8) AS order_number,
      MAX(s.id) AS seller_store_id,
...
-- 4. Comentarios
COMMENT ON FUNCTION get_inventario_b2c_agrupado IS 'Inventario B2C agrupado por producto con variantes en array';

-- =====================================================
-- VERIFICACIÓN
-- =====================================================
SELECT '✅ FUNCIÓN AGRUPADA CREADA' as resultado;
...
SELECT 
  ...
  precio_promedio,
  availability_status,
  jsonb_array_length(variantes) as cantidad_variantes,
  variantes
...
Ahora cada producto se agrupa en 1 tarjeta:
  - 1 tarjeta = 1 producto
  - variantes = array con detalles de cada variante
  - total_stock = suma de todas las variantes

📝 SIGUIENTE PASO:
Actualizar el hook TypeScript para usar la nueva función
