      ...
      if (existingCatalog) {
        ...
        const { error: updateError } = await supabase
          ...
          .update({
            nombre,
            descripcion,
            category_id: categoryId, // Permitir cambio de categoría
            source_order_id: orderId, // Guardar order_id del inventario B2B
            is_active: true,
            metadata: {
              delivery_time_days,
              shipping_cost,
        ...
        const { data: newCatalog, error: insertError } = await supabase
          ...
          .insert({
            ...
            source_order_id: orderId, // Guardar order_id del inventario B2B
            sku: variantes[0].sku, // Usar SKU de la primera variante como referencia
            nombre,
            descripcion,
            category_id: categoryId,
            precio_venta: 0, // Se maneja a nivel de variante
            precio_costo: 0, // Se calcula del inventario
