        ...
        <CardContent className="p-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {isExpress ? t('checkout.priority') : t('checkout.consolidated')}
                </p>
              </div>
            </div>
          ...
          <div className="space-y-1 mb-3 text-sm">
            <div className="flex justify-between text-muted-foreground">
              <span>{t('checkout.routeA')}</span>
              <span className="font-medium text-foreground">
                ${option.tramo_a_cost_per_kg.toFixed(2)}/kg
              </span>
            </div>
            <div className="flex justify-between text-muted-foreground">
              <span>{t('checkout.routeB')}</span>
              <span className="font-medium text-foreground">
          ...
          <div className="flex flex-wrap gap-2">
            {option.allows_oversize && (
              <Badge variant="secondary" className="text-xs">
                ✓ {t('checkout.oversizeProducts')}
              </Badge>
            )}
            {option.allows_sensitive && (
              <Badge variant="secondary" className="text-xs">
                ✓ {t('checkout.sensitiveProducts')}
              </Badge>
            )}
          ...
          {isExpress && expressDisabled && (
            <div className="mt-3 flex items-center gap-2 text-xs text-amber-600 bg-amber-50 p-2 rounded">
              ...
              <span>{t('checkout.oversizeUnavailable')}</span>
            </div>
          )}
  ...
  if (loading) {
    return (
      <div className="space-y-4">
        <h3 className="font-semibold text-lg">{t('checkout.shippingType')}</h3>
    ...
    return (
      <div className="p-4 bg-destructive/10 border border-destructive/30 rounded-lg">
        <div className="flex items-center gap-2 text-destructive">
          ...
          <span className="font-medium">{t('checkout.noLogisticsCoverage')}</span>
        </div>
        <p className="text-sm text-muted-foreground mt-1">
          {t('checkout.chooseDifferentAddress')}
        </p>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        ...
        {hasSensitiveProducts && (
          <Badge variant="outline" className="text-amber-600 border-amber-300">
            ⚠️ {t('checkout.containsSensitiveProducts')}
          </Badge>
        )}
      </div>
