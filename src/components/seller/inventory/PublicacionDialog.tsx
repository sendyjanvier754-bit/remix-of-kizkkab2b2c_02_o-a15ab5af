...
export function PublicacionDialog({ 
  ...
  // Margin calculation: (Sale Price - Total Cost) / Total Cost * 100
  // Total Cost = precio_b2b + logistics cost
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[95vw] max-w-md max-h-[90vh] overflow-y-auto overflow-x-hidden p-3">
        <DialogHeader className="pb-2">
          <DialogTitle className="text-sm">Configurar Publicación B2C</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-3 py-2 max-w-full">
          ...
          <div className="border-t pt-2 space-y-2.5">
            {/* Cost Breakdown */}
            <div>
              <h4 className="text-[11px] font-semibold mb-1.5 text-gray-700">Costos</h4>
              <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-2.5 space-y-1">
                <div className="flex justify-between items-center gap-2">
                  <span className="text-[10px] text-blue-900">Precio B2B (histórico)</span>
                  <span className="font-semibold text-xs text-blue-900">${item.precioB2B.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center gap-2">
                  <span className="text-[10px] text-blue-900">Logística (pagada: {item.weightKg}kg)</span>
                ...
                <div className="border-t border-blue-300 pt-1 mt-1">
                  <div className="flex justify-between items-center gap-2">
                    <span className="text-[11px] font-bold text-blue-950">Total Invertido</span>
                ...
                {item.costoLogistica === 0 && (
                  <div className="mt-1 text-[9px] text-yellow-800 bg-yellow-100 p-1 rounded">
                    ⚠️ Sin costo de logística registrado
                  </div>
                )}
              </div>
            </div>
            ...
            {item.precioSugeridoVenta && (
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-300 rounded-lg p-2.5">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex-1 min-w-0">
                    <p className="text-[9px] font-medium text-green-700 mb-0.5">💡 Sugerido</p>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className={`p-2 rounded-lg border ${
                margin < 0 ? 'bg-red-50 border-red-300' : 
                margin < 10 ? 'bg-yellow-50 border-yellow-300' : 
                'bg-green-50 border-green-300'
              }`}>
                <p className="text-[9px] font-medium text-gray-600 mb-0.5">Margen</p>
              ...
              <div className={`p-2 rounded-lg border ${
                gananciaUnitaria < 0 ? 'bg-red-50 border-red-300' : 'bg-green-50 border-green-300'
              }`}>
                <p className="text-[9px] font-medium text-gray-600 mb-0.5">Ganancia</p>
                <p className={`text-base font-bold leading-tight ${
                  gananciaUnitaria < 0 ? 'text-red-700' : 'text-green-700'
                }`}>
            ...
            {gananciaUnitaria < 0 && (
              <div className="bg-red-50 border border-red-300 rounded-lg p-1.5">
                <p className="text-[10px] text-red-800 font-medium leading-tight">
                  ⚠️ Pérdida ${Math.abs(gananciaUnitaria).toFixed(2)}/unidad
                </p>
              </div>
            )}
          ...
          <div className="border-t pt-2">
            <div className="flex items-center justify-between p-2.5 bg-gray-50 rounded-lg gap-2">
              <div className="flex-1 min-w-0">
                <Label htmlFor="isActive" className="font-semibold text-[11px]">Publicar</Label>
