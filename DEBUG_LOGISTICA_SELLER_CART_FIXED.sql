...
-- PASO 3: Verificar items en formato JSONB (como los pasa el hook)
WITH user_data AS (
  SELECT id as user_id FROM auth.users WHERE email = 'rsdorvil21@gmail.com'
),
cart_items_jsonb AS (
...
FROM cart_items_jsonb;


-- PASO 4: ⭐ ESTE ES EL MÁS IMPORTANTE - RESULTADO REAL
-- Llamar a la función get_cart_shipping_cost directamente
WITH user_data AS (
  SELECT id as user_id FROM auth.users WHERE email = 'rsdorvil21@gmail.com'
),
cart_items_jsonb AS (
...
SELECT 
  ...
  ROUND((get_cart_shipping_cost(items)->>'total_weight_kg')::numeric, 3) as "Peso Total (kg)",
  (get_cart_shipping_cost(items)->>'weight_rounded_kg')::numeric as "Peso Redondeado",
  ROUND((get_cart_shipping_cost(items)->>'base_cost')::numeric, 2) as "Costo Base",
  ROUND((get_cart_shipping_cost(items)->>'oversize_surcharge')::numeric, 2) as "Surcharge Oversize",
  ROUND((get_cart_shipping_cost(items)->>'dimensional_surcharge')::numeric, 2) as "Surcharge Dimensional",
  ROUND((get_cart_shipping_cost(items)->>'extra_cost')::numeric, 2) as "Costo Extra",
  ROUND((get_cart_shipping_cost(items)->>'total_cost_with_type')::numeric, 2) as "💰 TOTAL USD",
  (get_cart_shipping_cost(items)->>'shipping_type_display')::text as "Tipo Envío"
FROM cart_items_jsonb;


-- PASO 5: Verificar pesos de los productos individuales
WITH user_data AS (
  SELECT id as user_id FROM auth.users WHERE email = 'rsdorvil21@gmail.com'
)
SELECT 
...
-- 4. ⭐ ¿Qué está devolviendo la función? (EL MÁS IMPORTANTE)
