-- =====================================================
-- ACTUALIZAR get_inventario_b2c_agrupado PARA INCLUIR CATEGORIA
-- =====================================================

DROP FUNCTION IF EXISTS get_inventario_b2c_agrupado(UUID, TEXT, INTEGER) CASCADE;

-- Primero actualizar el tipo compuesto si es necesario
...
CREATE TYPE inventario_b2c_producto_agrupado AS (
  ...
  variantes JSONB,
  categoria_id UUID  -- ✅ Nuevo campo
);

-- Recrear la función incluyendo categoria_id
...
BEGIN
  ...
  WITH variantes_agrupadas AS (
    SELECT 
      ...
      p.categoria_id,  -- ✅ Obtener categoria_id del producto original
      jsonb_agg(
        jsonb_build_object(
          'order_item_id', oi.id,
          'variant_id', pv.id,
  ...
  SELECT 
    ...
    ultima_actualizacion::TIMESTAMPTZ,
    variantes::JSONB,
    categoria_id::UUID  -- ✅ Incluir en el SELECT
...
SELECT '✅ FUNCIÓN get_inventario_b2c_agrupado ACTUALIZADA con categoria_id' as resultado;
