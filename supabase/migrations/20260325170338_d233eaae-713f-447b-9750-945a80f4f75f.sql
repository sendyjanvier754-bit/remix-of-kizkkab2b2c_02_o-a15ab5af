...
BEGIN
  ...
  -- ── INSERT new products (without variants / is_parent) ──
  ...
  SELECT
    ...
    vp.precio_b2b,        -- precio_venta = B2B base (PVP se calcula al mostrar)
    vp.precio_b2b,        -- precio_costo
    vp.precio_b2b,        -- precio_b2b_base
    0,                    -- costo_logistica (se calcula en runtime)
  ...
  SELECT
    ...
    0,
    COALESCE(vv.stock, 0),
    COALESCE(vv.images::jsonb, '[]'::jsonb),
    TRUE,
    'b2b_auto_sync',
  ...
  v_added := v_added + (SELECT COUNT(*) FROM seller_catalog WHERE FALSE); -- placeholder
  GET DIAGNOSTICS v_updated = ROW_COUNT;
  v_added := v_added + v_updated;
  v_updated := 0;
